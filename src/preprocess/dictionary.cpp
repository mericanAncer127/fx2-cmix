...
Dictionary::Dictionary(FILE* dictionary, bool encode, bool decode) {
    ...
    for (size_t i = 0; i < sorted_ascii.size() && i < 12; ++i) {
        for (size_t j = 0; j < sorted_ascii.size() && j < 12; ++j) {
            for (size_t k = 0; k < sorted_ascii.size() && k < 12; ++k) {
